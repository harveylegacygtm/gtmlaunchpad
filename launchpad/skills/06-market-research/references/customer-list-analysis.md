...
1. This is your ICP and the research on it. The one-paragraph ICP and anti-ICP,
   the 4-layer profile, pains and language in real words with sources. (Standard
   06 output.)
2. This is your offer and the market research around it. Their service offering
   ...
   (Uses the user's baseline. Feeds 07-offer.)
...
2. Plug-in ICP filters. A ready-to-use block of firmographic + signal filters (the
   ranked triggers as list-building criteria) formatted to drop straight into the
   10-lead-list skill. This is the bridge from "here are the patterns" to "here is
   the next list."
3. Updated ICP doc. Fold the findings into the standard 06 artifact so the
   Signal-to-Offer Map and the 0 to 100 scoring rubric reflect the real customer
   patterns. A trigger that 9 of 12 customers share earns real weight in the
   rubric.
